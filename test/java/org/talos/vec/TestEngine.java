...
public class TestEngine {
    ...
    public static void exec(final Testable t, final TestableCallback cb) throws Throwable {
        ...
        final TestableCallback ncb = new TestableCallback() {
            ...
            public void excepted() {
                cb.excepted();
                try {
                    this.realStatus = cb.realStatus;
                    this.realMessage = cb.realMessage;
                    this.realType = cb.realType;
                    this.realPayload = cb.realPayload;
                    validate();
                } catch (Throwable e) {
                    errMap.put(t, e);
    ...
    public static Testable rmk(final String vkeySource, final String vkeyTarget) {
        return new Testable() {
            @Override
            public void test(TestableCallback cb) {
                logger.info(String.format("%s %s %s", "rmk", vkeySource, vkeyTarget));
                engine.rmk(cb, vkeySource, vkeyTarget);
                try {
                    Thread.sleep(10);
                } catch (InterruptedException e) {
                    e.printStackTrace();
