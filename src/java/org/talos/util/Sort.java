...
public class Sort {
     ...
     * 3501671, 8810089, 21521774, 58548857, 157840433, 410151271, 1131376761,
     * 2147483647};
     */
    ...
    public static <T extends Comparable<? super T>> void mergeSort(T[] arr) {
        /*
         * if ( arr.length < 15 ) { insertionSort( arr ); return; }
         */
    ...
    private static <T extends Comparable<? super T>> void mergeSort(T[] arr, T[] tmpArr, int left, int right) {
        ...
        // loop instead
        /*
         * int len = 2, pos; int rpos, offset, cut; while ( len <= right ) { pos
         * = 0; offset = len / 2; while ( pos + len <= right ) { rpos = pos +
         * offset; merge( arr, tmpArr, pos, rpos, rpos + offset - 1 ); pos +=
         * len; }
         * 
         * //merge the rest cut = pos + offset; if ( cut <= right ) merge( arr,
         * tmpArr, pos, cut, right );
         * 
         * len *= 2; } merge( arr, tmpArr, 0, len / 2, right );
         */
    }
    ...
    private static <T extends Comparable<? super T>> void merge(T[] arr, T[] tmpArr, int lPos, int rPos, int rEnd) {
        ...
        while (lPos <= lEnd)
            tmpArr[tPos++] = arr[lPos++];
        // copy the rest elements of the right half subarray. (only one loop
        // will be execute)
        while (rPos <= rEnd)
            tmpArr[tPos++] = arr[rPos++];
