...
public class JcsegTaskConfig {
    ...
    public void resetFromPropertyFile(String proFile) throws IOException {
        ...
        if (proFile == null) {
            /*
             * 1.load the the jcseg.properties located with the jar file. 2.load
             * the jcseg.propertiess from the classpath. 3.load the
             * jcseg.properties from the user.home
             */
            ...
            /*
             * jcseg properties file loading status report, show the crorrent
             * properties file location information . <br />
             * 
             * @date 2013-07-06
             */
            if (!jcseg_properties) {
        ...
        for (String fpath : lexPath) {
            f = new File(fpath);
            if (!f.exists())
                throw new IOException("Invalid sub lexicon path " + fpath + " for lexicon.path in jcseg.properties");
