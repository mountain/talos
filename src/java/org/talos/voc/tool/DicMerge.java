...
public class DicMerge {
    ...
    public static int merge(File[] srcFiles, File dstFile) throws IOException {
        ...
        HashMap<String, Token> entries = new HashMap<String, Token>();
            ...
            while ((line = reader.readLine()) != null) {
                ...
                if (line.indexOf('/') == -1) { // simple word
                    if (!entries.containsKey(line))
                        entries.put(line, new Token(line, 1));
                    continue;
                }
                ...
                if (word == null) {
                    ...
                    // check the pinyin
                    if (word.pinyin() == null && !splits[2].equals("null"))
                        word.pinyin(splits[2]);
        ...
        for (int j = 0; j < keys.length; j++) {
            ...
            isb.clear();
            isb.append(word.value()); // word
            isb.append('/');
            if (word.tags() == null) // part of speech
                isb.append("null");
            else {
                String[] ps = word.tags();
            ...
            if (word.pinyin() == null) // pinyin
                isb.append("null");
            else
                isb.append(word.pinyin());

            isb.append('/');

            if (word.synm() == null) // synonyms
                isb.append("null");
            else {
                String[] syn = word.synm();
            ...
            if (word.type() == 2) { // single word degree
                isb.append('/');
                isb.append("" + word.freq());
            }
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            for (int j = 0; j < files.length; j++) {
                ...
                int ct = merge(dstFiles, new File(dstDir.getAbsolutePath() + "/" + files[j].getName()));
                if (ct > 0) {
                    counter++;
