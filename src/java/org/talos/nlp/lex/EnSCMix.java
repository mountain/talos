...
public class EnSCMix {
     ...
     * 
     * static { enKeepChar = new HashMap<Character, Character>( ( int
     * )(EN_KEEP_CHARS.length / 1.7) + 1, 0.85f ); //set the keep char's keep
     * status for ( int j = 0; j < EN_KEEP_CHARS.length; j++ )
    ...
    public static boolean isEnLetter(int u) {
        if (u > 65280)
            u -= 65248; // make full-with half-width
